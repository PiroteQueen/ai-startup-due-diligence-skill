...
- Extended vocab alignment to the reference rule docs: `coverage-stage-model.md` (the authoritative stage-dimension definitions) and the AI-status line in `ai-product-strategy.md` are now checked to equal `vocab.py`, so the protocol docs can no longer define a vocabulary that drifts from the code. (Failure-type prose in `source-access-strategy.md` stays human-readable by design; its machine contract is enforced via the template and fixtures.)
